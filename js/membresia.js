/**
 * Gate de acceso por clave de miembro (port del sistema de GamerCB2026/PruebaCEGCD).
 * La clave SOLO vive en el Worker (secret ACCESS_CODE). Nunca en este archivo.
 * Sesión = { token, exp, ts } firmada por el Worker (HMAC exp.hmac).
 * Offline: si el Worker no responde, la sesión local vigente sigue abriendo el editor.
 */
const MEMBRESIA_CONFIG = {
  ...
  appName: "Chart Editor GCD Plus",
  channelHandle: "@GamerCB2026",
  creatorChannelId: "UCA-nQeq52AxDtdQYWVi3dxQ" // solo para el link "Unirme"
};
...
function _memSessionUrl() {
  ...
  // Fallback: el mismo endpoint acepta { token }
...
function _memEntrarDev() {
  ...
  // Desbloqueo solo-dev local (sin token de servidor). Recargar el gate lo revalida.
...
async function _memVerificarSesionRemota(sesion) {
  ...
  // Token dev local: válido sin red (solo localhost)
  if (sesion.token === "dev.local" && _memEsLocalHost()) return true;
  if (sesion.legacy || sesion.token === "legacy.worker") return false; // fuerza re-login
...
async function iniciarSistemaMembresia() {
  ...
  if (sesion && _memConfigCompleta()) {
    ...
    // Inválida o expirada → fuera; si fue fallo de red, conserva el token
    // local para que un reintento (offline) no desaloje a nadie.
    if (sesion.token !== "dev.local") _memBorrarSesion();
  }
